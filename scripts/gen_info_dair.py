...
def load_data(dair_root, token, load_gt):
...
def generate_info_dair(dair_root, split):    
    split_infos = mmcv.load("data/single-infrastructure-split-data.json")
    split_list = split_infos[split]
    if "raw" in split_infos.keys() and split == 'train':
        split_list = split_infos['train'] + split_infos['val']
        random.shuffle(split_list)

    infos = list()
    for sample_id in tqdm(split_list):
        token = "image/" + sample_id + ".jpg"
        load_gt = True if sample_id not in split_infos['val'] else False
        r_velo2cam, t_velo2cam, camera_intrinsic, gt_names, gt_boxes, img_pth = load_data(dair_root, token, load_gt)
...
def main():
    ...
    trainval_infos = train_infos
    for info in tqdm(val_infos):
        info["ann_infos"] = list()
        trainval_infos.append(info)
    random.shuffle(trainval_infos)
    mmcv.dump(trainval_infos, './data/dair-v2x-i/dair_12hz_infos_trainval.pkl')
