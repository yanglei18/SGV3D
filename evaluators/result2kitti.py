...
def result2kitti(results_file, results_path, dair_root, gt_label_path, demo=False):
    ...
    for sample_token in tqdm(results.keys()):
        ...
        calib_file = os.path.join(dair_root, "training/calib", "{:06d}".format(sample_id) + ".txt")
        Tr_velo2cam, camera_intrinsic = load_calib_dair(calib_file)
        r_velo2cam, t_velo2cam = Tr_velo2cam[:3,:3], Tr_velo2cam[:3,3][:, np.newaxis]
        ...
        for pred in preds:
            ...
            box2d = bbbox2bbox(box, Tr_velo2cam, camera_intrinsic)
